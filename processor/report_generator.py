    ...
    if api_key and context_text.strip():
        try:
            prompt = (
                ...
                "이 뉴스들을 분석하여 파워포인트 슬라이드 덱 형태의 마크다운 리포트를 작성해 주세요.\n\n"
                "중요: 각 슬라이드의 구분은 반드시 '---' (하이픈 3개)로 구분해야 합니다. 슬라이드가 바뀔 때마다 빈 줄에 '---'를 적어주세요.\n\n"
                "## [슬라이드 구성 양식]\n"
                "### 슬라이드 1: 타이틀 (Header)\n"
                f"# [일간] 글로벌 벤처캐피탈·스타트업 뉴스 브리핑\n"
                f"- 발송일자: {today_str}\n"
                "- 작성자: AI 애널리스트\n\n"
                "---\n"
                "### 슬라이드 2~N: 기업 분석 (Company Analysis & Deal Flow)\n"
                "※ 제공된 뉴스 중 특정 '기업'과 관련된 기사들을 기업 1개당 1장의 슬라이드로 쪼개어 아래 양식으로 정리해 주세요.\n"
                "## [기업명 (국가/라운드)]\n"
                "- **기사 일자**: / **중요도**: / **출처**:\n"
                "- **투자 금액**: \n"
                "- **핵심 요약**: 한국어 번역 헤드라인 및 주요 비즈니스 모델\n"
                "- **기업 분석**: (1) Executive summary (2) 산업 분석 (3) 사업 분석 등 (투심보고서 양식 차용)\n\n"
                "---\n"
                "### 슬라이드 M~K: 뉴스 브리핑 (VC & Policy News Briefing)\n"
                "※ 벤처캐피탈 시장, 펀드, 정책 등 일반 '뉴스' 성격의 기사들을 주제별(VC 동향, 정부 정책 등)로 묶어서 1~2장의 슬라이드로 정리해 주세요.\n"
                "## [주제 카테고리명]\n"
                "- **기사 제목 (일자/출처)**\n"
                "  - 주요 내용 요약\n\n"
